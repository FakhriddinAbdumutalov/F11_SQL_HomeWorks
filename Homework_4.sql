...
select sum(purch_amt) as TotalPurchase from Inventory.Orders;
...
select avg(purch_amt) as AveragePurchase from Inventory.Orders;

--3. From the following table, write a SQL query that counts the number of unique salespeople.
--Return number of salespeople.  

select distinct count(salesman_id) as UniqueSalespeoaple from Inventory.Salesman;

--4. From the following table, write a SQL query to count the number of customers.
--Return number of customers.

select count(customer_id) as CustomerNumber from Inventory.Customer;
...
select max(purch_amt) MaximumPurchase from Inventory.Orders;

--7. From the following table, write a SQL query to find the minimum purchase amount.

select min(purch_amt) MinimumPurchase from Inventory.Orders;
...
--11. From the following table, write a SQL query to determine the highest purchase amount
--made by each salesperson on '2012-08-17'.
...
--amount by each customer on a particular order date. Filter the result by highest order (purchase) amount above 2000.00.
...
--amount in the range 2000 - 6000 (Begin and end values are included.) by combination of each customer and order date.
--Return customer id, order date and maximum purchase amount.
...
--amount based on the combination of each customer and order date.
...
--15. From the following table, write a SQL query to determine the maximum orderamount for each customer.
...
select count(salesman_id) as count from Inventory.Salesman;

--20. From the following table, write a SQL query to count the number of orders based on 
--the combination of each order dateand salesperson.
...
select count(*) as count from item_mast
where pro_price >= 350;
...
select avg(pro_price) as AvgPrice, pro_com from item_mast
group by pro_com;
...
--    dpt_code int primary key,     -- Unique Department Code
--    dpt_name varchar(50) not null, -- Department Name
--    dpt_allotment int not null    -- Budget/Allotment for the Department
--);
...
select sum(dpt_allotment) as Total from emp_department;
